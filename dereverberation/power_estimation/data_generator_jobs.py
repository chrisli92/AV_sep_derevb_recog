...
class CHDataset(object):
    def __init__(self, stage, inference_job_num, job):
        ...
        elif self.stage == 'replay':
            self.info = pickle.load(open(replay_path, 'rb'), encoding='utf-8') # for stats
            self.info_ref = pickle.load(open(test_path, 'rb'), encoding='utf-8')  # use test_path clean  as a ref since replay_path has no clean ref
            new_wav_scp_path = replay_wavscp   # input replay wav
        elif self.stage == 'lrs3_test':
            self.info = pickle.load(open(lrs3_test_path, 'rb'), encoding='utf-8') # for stats
            new_wav_scp_path = lrs3_test_wavscp   # input replay wav
        ...
        if new_wav_scp:
            with open(new_wav_scp_path, 'r', encoding='utf-8') as fr:
                ...
                for line in lines:
                    line = line.strip()
                    line_lst = line.split()
                    if line_lst[0] in self.wav_list: # only save this chunk new wav list
    ...
    def get_data(self, wav_file):
        ...
        if self.stage == 'replay':
            wav_ref_path = self.info_ref[wav_file]['wav_path']
        ...
        directions = []
        if self.stage == 'replay':
            spk_doa = [float(doa) for doa in spk_doa]
            
        for x in range(nspk_in_wav):
            if self.stage == 'replay':
                directions.append((spk_doa[x] + 180) * np.pi / 180)
            else:
        ...
        if self.stage == 'replay':
            wav_ref, _ = librosa.load(wav_ref_path, sr=sampling_rate, mono=False)
            ref_clean = wav_ref[n_mic]
        else:
            ref_clean = wav[n_mic]
        ...
        # ref_clean = wav[n_mic] # clean data as dervb ref
        # tgt_50ms_rvb = wav[n_mic + 1]  # for rvb_only
        ...
        # if self.stage != 'tr':
        #     return mix_wav, tgt_clean, lip_video, tgt_50ms_rvb
        # else:
...
class CHDataLoader(object):
    ...
    def next_batch(self):
        ...
        for i in range(n_begin, n_end, 1):
            ...
            mix, tgt_clean, lip_video, tgt_50ms_rvb = self.dataset.get_data(wav_file)
