...
def get_filter_matrix_conj(
    ...
    if use_diag_loading:
        eye = torch.eye(correlation_matrix.size(-1), dtype=correlation_matrix.dtype, device=correlation_matrix.device)
        # import pdb; pdb.set_trace()
        eye = eye.reshape(1, correlation_matrix.size(-1), correlation_matrix.size(-1)).repeat(F, 1, 1)
        with torch.no_grad():
            epsilon = FC.trace(correlation_matrix).real.abs()[..., None, None] * diag_loading_ratio
...
def wpe_one_iteration(Y: ComplexTensor,
    ...
    filter_matrix_conj = get_filter_matrix_conj(
        correlation_matrix, correlation_vector, diag_loading_ratio=diag_loading_ratio)
