...
from pt_audio_fea import DFComputer, STFT, iSTFT
from params import *
...
class DervbNet(nn.Module):
    ...
    def forward(self, all_x):
        ...
        # Feature extractor
        # audio_fea: (B*C , F, T), mag_rev: (B, C, F, T), phase_rev: (B, C, F, T)
        ...
        ###########multi-channel dervb ########
        # repeat single-channel to multi-channel
        V_B, V_T, V_F = video_fea.shape
        video_fea = video_fea.unsqueeze(1).expand(V_B, n_mic, V_T, V_F).contiguous().view(V_B*n_mic, V_T, V_F)
        ###########multi-channel dervb ########
        ...
        if model_type == 'LSTM':
            ...
            ###########multi-channel dervb ########
            # (B*C, F, T) -> (B, C, F, T)
            e_real = e_real.view(x.shape[0], n_mic, e_real.shape[1], e_real.shape[2])
            e_imag = e_imag.view(x.shape[0], n_mic, e_imag.shape[1], e_imag.shape[2])  
            ###########multi-channel dervb ########
        else:
            raise ValueError(f"Not support model type = {model_type}")
        ...
        # import pdb; pdb.set_trace()
        # (B, C, F, T) -> (B, C, F, T) 
        mask_real = e_real.masked_fill(make_pad_mask(lens, e_real, length_dim=3), 0)
        mask_imag = e_imag.masked_fill(make_pad_mask(lens, e_imag, length_dim=3), 0)
        ...
        if self.normalization:
            # Normalize along T
            # denominator: (B, C, F, T) -> (B, C, F) -> (B, C, F, 1) for broadcast
            # mask: (B, C, F, T)
            # import pdb; pdb.set_trace()
