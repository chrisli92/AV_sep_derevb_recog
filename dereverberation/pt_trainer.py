...
class Trainer(object):
    ...
    def train(self, data_loader):
        ...
        for data in data_loader:
            ...
            # import pdb; pdb.set_trace() 
            # self.nnet.register_full_backward_hook(self.nnet.backward_hook)
            ...
            # modules = self.nnet.named_children()
            ...
            #     # if name == 'df_computer.ln_LPS.weight':
            #     # import pdb; pdb.set_trace()
            #     if param != None and param.grad_fn != None:
            #         print('name:{}'.format(name))
            #     if param.requires_grad:
            #         print('name:{}'.format(name))
            #         # print('grad_fn:{}'.format(param.grad_fn))
            #         print('param grad:{}'.format(torch.mean(param.grad)) if param.grad != None else 0)
            
            # import pdb; pdb.set_trace()
            progress = stats.count("loss")  # batch cnt
            # if not progress % self.logging_period:
            #     self.logger.info("Process {:d}-th batch and its loss mse: {:.3f}, sisnr: {:.3f}, lfb mse: {:.3f} ".format(progress, loss.item(), loss_sisnr.item() if loss_sisnr else 0, loss_lfb.item() if loss_lfb else 0))
            ## accum_grad
            ...
            if not progress % (self.accum_grad*self.logging_period):
                self.logger.info("Process {:d}-th batch and its stft mse: {:.3f}, sisnr: {:.3f}, lfb mse: {:.3f}, back loss is : {:.3f}, forward_count:{} ".format(progress, loss_mse.item(), loss_sisnr.item(), loss_lfb.item(), loss.item(), self.forward_count))
            self.forward_count = 0
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for data in data_loader:
                ...
                stats.add("loss", value=loss_mse.item(), cnt=this_batch_size)
                stats.add("sisnr", value=loss_sisnr.item() if loss_sisnr else 0, cnt=this_batch_size)
                stats.add("lfb", value=loss_lfb.item() if loss_lfb else 0, cnt=this_batch_size)

        return stats.value('loss'), stats.count("loss"), timer.elapsed(), stats.value('sisnr'), stats.value('lfb')
    ...
    def run(self, train_loader, dev_loader, num_epochs=100):
        ...
        while self.cur_epoch < num_epochs:
            ...
            cv_loss, cv_batch, cv_cost, cv_sisnr_loss, cv_lfb_loss = self.evaluate(dev_loader)
            stats["cv"] = "dev_loss = {:+.5f}, ({:.2f}s/{:d}), sisnr= {:.5f}, lfb= {:.5f}".format(cv_loss,
                                                                                                  cv_cost,
                                                                                                  cv_batch,
                                                                                                  cv_sisnr_loss,
                                                                                                  cv_lfb_loss)
...
class DervbTrainer(Trainer):
    ...
    def compute_loss(self, data_all):
        ...
        with torch.no_grad():
            ...
            data["ref"] = data["ref"] # ref is overlap and noise speech which is wav format.
        
        # import pdb; pdb.set_trace()
        lens = data["ori_wav_len"]
        if est_power:
            power_lst = torch.nn.parallel.data_parallel(self.nnet,
                                                                [data["mix"],
                                                                 data["lip_video"],
                                                                 data["ref"],
                                                                 data["ori_wav_len"]],
                                                               device_ids=self.gpuid)
            loss_mse =  self.mse_loss(power_lst[0], power_lst[1], lens) * scale
            print(f"loss_mse: {loss_mse}")
            return loss_mse, None, None
        else:
            ...
            loss_mse = (self.mse_loss(real_est, real_ref, lens) + self.mse_loss(imag_est, imag_ref, lens)) * scale
            # import pdb; pdb.set_trace()
            ...
            # if torch.any(torch.isnan(loss_mse)):
            #     import pdb; pdb.set_trace()
            
            print(f"loss_mse {loss_mse}, loss_sisnr : {loss_sisnr}, loss_lfb: {loss_lfb}")
