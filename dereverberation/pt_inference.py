...
class NNetComputer(object):
    def __init__(self, save_model_dir=model_save_dir, gpuid=gpuid):
        print("model name is {}".format(decode_model_name))
        self.model_name = decode_model_name
        nnet = DervbNet()
        # cpt_fname = os.path.join(save_model_dir, self.model_name)
        cpt_fname = model_abs_path
        cpt = torch.load(cpt_fname, map_location="cpu")
        nnet.load_state_dict(cpt["model_state_dict"])
        # print("Load checkpoint from {}, epoch {:d}".format(cpt_fname, cpt["epoch"]))
...
def run(eval_type):
    ...
    for item_idx in range(len(dataset.wav_list)):
        ...
        pesq = pesq = ra.get_PESQ(est_s1, s1)
        stoi = 0
        srmr = 0
        if all_metrics:
            stoi = ra.get_STOI(est_s1, s1)
            srmr = ra.get_SRMR(est_s1, s1)
        ...
        pc.update_performance(wav_file, snr1, ad, spk_num, spk_rt60, metric='SI-SDRi')
