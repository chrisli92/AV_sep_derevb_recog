...
# logger.info(f"hop_size:{HOP_SIZE}, diag_loading_ratio: {diag_loading_ratio}, taps: {taps}, delay: {delay}")
logger.info(f"hop_size:{HOP_SIZE}, diag_loading_ratio: {diag_loading_ratio}, mvdr")

if inference_dataset_name == 'replay':
    print(f"this test is for replay")
    from data_generator_replay_jobs import CHDataset, CHDataLoader
else:
