...
class Trainer(object):
    def __init__(self,
        ...
        # self.num_params_tcn =sum([param.nelement() for param in nnet.audio_blocks.parameters()]) / 10.0 ** 6
...
#                w_mat = i[1].view(factor, -1)
 ...
 #                                            reduce=True, size_average=False)
    ...
    def train(self, data_loader):
        ...
        for data in data_loader:
            ...
            if not progress % self.logging_period:
                # self.logger.info("Processed {:d} batches, loss sisnr{:.2f}, loss lfb {:.2f}, loss sisnr_noise {:.2f} ...".format(progress, loss_sisnr.item() if 'SISNR' in loss_type else 0, loss_lfb.item() if 'LFB' in loss_type else 0, loss_sisnr_noise.item() if 'LFB' in loss_type else 0))#, orthogonal_loss.item()))
                self.logger.info("Processed {:d} batches, loss sisnr{:.2f}, loss lfb {:.2f}, loss sisnr_noise {:.2f} ...".format(progress, loss_sisnr.item() if 'SISNR' in loss_type else 0, loss_lfb.item(), loss_sisnr_noise.item() if 'SISNR_NOISE' in loss_type else 0))#, orthogonal_loss.item()))
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for data in data_loader:
                loss_sisnr, loss_lfb, loss_sisnr_noise = self.compute_loss(data)
                all_loss = 0
                if 'LFB' in loss_type:
                    all_loss += loss_lfb
                if 'SISNR' in loss_type:
                ...
                # stats.add("loss", all_loss.item())
                stats.add("sisnr_loss", loss_sisnr.item())
                stats.add("noise_loss", loss_sisnr_noise.item())
        return stats.value("loss"), stats.count("loss"), timer.elapsed(), stats.value("sisnr_loss"), stats.value("noise_loss")
    ...
    def run(self, train_loader, dev_loader, num_epochs=50, warm_up_epochs=20):
        ...
        while self.cur_epoch < num_epochs:
            ...
            cv_loss, cv_batch, cv_cost, cv_sisnr, cv_noise  = self.evaluate(dev_loader)
            stats["cv"] = "dev = {:+.4f}({:.2f}s/{:d}) | sisnr = {:+.4f} | sisnr_noise = {:+.4f}".format(
                cv_loss, cv_cost, cv_batch, cv_sisnr, cv_noise)
            stats["scheduler"] = ""
...
class SiSnrTrainer(Trainer):
    ...
    def compute_loss(self, data):
        ...
        ests, ests_noise, estspec = torch.nn.parallel.data_parallel(self.nnet, [data["mix"], data["src_doa"], data["spk_num"], data["lip_video"], None],  device_ids=self.gpuid)
        #import pdb; pdb.set_trace()
        ...
        #print(loss_sisnr)
        # si-snr
        print(f"loss_sisnr : {loss_sisnr}, loss_lfb: {loss_lfb}, loss_sisnr_noise:{loss_sisnr_noise}")
        return loss_sisnr, loss_lfb, loss_sisnr_noise
