...
def batchfy_by_seq(
    ...
    while True:
        ...
        vilen = int(info[ikey][iaxis]["vshape"][0])
        ailen = int(info[ikey][iaxis]["ashape"][0])
        olen = (
            int(info[okey][oaxis]["shape"][0])
            if oaxis >= 0
            else max(map(lambda x: int(x["shape"][0]), info[okey]))
        )
        factor = max(int(ailen / max_length_in), int(olen / max_length_out))
...
def make_batchset(
    data,
    batch_size=0,
    max_length_in=float("inf"), # 512
    max_length_out=float("inf"), # 150
    num_batches=0,
    min_batch_size=1,
    shortest_first=False,
    ...
    if count not in BATCH_COUNT_CHOICES: #count=auto, ['auto', 'seq', 'bin', 'frame'] 
        raise ValueError(
            f"arg 'count' ({count}) should be one of {BATCH_COUNT_CHOICES}"
        )
    if batch_sort_key not in BATCH_SORT_KEY_CHOICES:  #batch_sort_key=input, ['input', 'output', 'shuffle']
        raise ValueError(
    ...
    for d in category2data.values():
        ...
        sorted_data = sorted(
            d.items(),
            key=lambda data: int(data[1][batch_sort_key][batch_sort_axis]["ashape"][0]),
