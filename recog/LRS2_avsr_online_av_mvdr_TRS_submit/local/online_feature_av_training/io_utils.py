...
class LoadInputsAndTargets(object):
    ...
    def __init__(
        self,
        mode="asr",
        preprocess_conf=None, # 'conf/specaug.yaml'
        load_input=True,
        load_output=True,
        sort_in_input_length=True,
        use_speaker_embedding=False,
        use_second_target=False,
        preprocess_args=None, # {'train': True}
    ...
    def __call__(self, batch, return_uttid=False):
        ...
        ax_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        vx_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    # ax = self._get_from_loader(
                    #     filepath=inp["afeat"], filetype=inp.get("filetype", "mat")
                    # )
                    ax = self._get_from_loader(
                        filepath=inp["afeat_wav"], filetype=inp.get("filetype", "sound")
                    )
                    if ax.ndim != 1:
                        ax = ax[:, 0] # select first channel wav of 15 channels to train
                    # import pdb; pdb.set_trace()
                    vx = self._get_from_loader(
                        filepath=inp["vfeat"], filetype=inp.get("filetype", "npy")
                    )
                    
                    ax_feats_dict.setdefault(inp["name"], []).append(ax)
                    ax_feats_dict.setdefault("ilens", []).append(inp["ashape"][0])
    
                    vx_feats_dict.setdefault(inp["name"], []).append(vx)
        ...
        if self.mode == "asr":
            return_batch, uttid_list = self._create_batch_asr(
                ax_feats_dict, vx_feats_dict, y_feats_dict, uttid_list
            )
        elif self.mode == "tts":
            _, info = batch[0]
        ...
        if self.preprocessing is not None:
            ...
            for x_name in return_batch.keys():
                if x_name.startswith("ainput"):
                    print(f"LGN specaug: {x_name}")
    ...
    def _create_batch_asr(self, ax_feats_dict, vx_feats_dict, y_feats_dict, uttid_list):
        ...
        axs = list(ax_feats_dict.values())
        vxs = list(vx_feats_dict.values())

        if self.load_output:
            ys = list(y_feats_dict.values())
            assert len(axs[0]) == len(ys[0]), (len(axs[0]), len(ys[0]))
            assert len(vxs[0]) == len(ys[0]), (len(vxs[0]), len(ys[0]))
            ...
            nonzero_idx = list(range(len(axs[0])))

        if self.sort_in_input_length:
            # sort in input lengths based on the first input
            nonzero_sorted_idx = sorted(nonzero_idx, key=lambda i: -len(axs[0][i]))
        else:
            nonzero_sorted_idx = nonzero_idx

        if len(nonzero_sorted_idx) != len(axs[0]):
            logging.warning(
        ...
        axs = [[x[i] for i in nonzero_sorted_idx] for x in axs]
        vxs = [[x[i] for i in nonzero_sorted_idx] for x in vxs]
        uttid_list = [uttid_list[i] for i in nonzero_sorted_idx]

        # x_names = list(x_feats_dict.keys())
        x_names = list(ax_feats_dict.keys())
        if self.load_output:
            ...
            return_batch = OrderedDict(
                [
                    *[("a" + x_name, ax) for x_name, ax in zip(x_names, axs)],
                    *[("v" + x_name, vx) for x_name, vx in zip(x_names, vxs)],
                    *[(y_name, y) for y_name, y in zip(y_names, ys)],
                ]
            )
        else:
            # return_batch = OrderedDict([(x_name, x) for x_name, x in zip(x_names, axs)])
            return_batch = OrderedDict(
                [
                    *[("a" + x_name, ax) for x_name, ax in zip(x_names, axs)],
                    *[("v" + x_name, vx) for x_name, vx in zip(x_names, vxs)],
