...
class LoadInputsAndTargets(object):
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    #    "filetype": "hdf5",
                    #    "name": "input1", ...}], ...}
                    # import pdb; pdb.set_trace() # check sound file and wav file float32 or float 64, please keep the same.
                    # just for compare with online extract fbank feature even though online one has no preproces (such as pre-amphasis) before getting fbank.
                    x_kadli_fbank_no_pitch_no_cmvn = self._get_from_loader( 
                        filepath=inp["feat"], filetype=inp.get("filetype", "mat")
                    )
                    # print(f"uttid: {uttid} --> raw fbank, no pitch, no cmvn : {x_kadli_fbank_no_pitch_no_cmvn}")
                    x = self._get_from_loader(
                        filepath=inp["feat_wav"], filetype=inp.get("filetype", "sound")
                    )
                    # xx = self._get_from_loader(filepath=inp["feat_wav"], filetype=inp.get("filetype", "wav"))
                    x_feats_dict.setdefault(inp["name"], []).append(x)
                    x_feats_dict.setdefault("ilens", []).append(inp["shape"][0])
    ...
    def _get_from_loader(self, filepath, filetype):
        ...
        if filetype == "hdf5":
            ...
            # Assume PCM16
            if not self.keep_all_data_on_mem:
                array, _ = librosa.load(filepath, sr=16000, mono=False)
                return array
            if filepath not in self._loaders:
                array, _ = librosa.load(filepath, sr=16000, mono=False)
                self._loaders[filepath] = array
