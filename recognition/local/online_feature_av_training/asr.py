...
#     torch_snapshot,
# )
from espnet.online_feature_av_training.asr_utils import (
    CompareValueTrigger,
    adadelta_eps_decay,
    add_results_to_json,
    format_mulenc_args,
...
# )
from espnet.online_feature_av_training.asr_init import (
    freeze_modules,
    load_trained_model,
    load_trained_modules,
)
...
from espnet.online_feature_av_training.io_utils import LoadInputsAndTargets

# from espnet.utils.training.batchfy import make_batchset
from espnet.online_feature_av_training.batchfy import make_batchset
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        if self.grad_noise:
            from espnet.online_feature_av_training.asr_utils import add_gradient_noise
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        ilens = np.array(ilens_lst)

        # perform padding and convert to tensor
        # currently only support real number
        if axs[0].dtype.kind == "c":
            axs_pad_real = pad_list(
                [torch.from_numpy(x.real).float() for x in axs], 0
            ).to(device, dtype=self.dtype)
            axs_pad_imag = pad_list(
                [torch.from_numpy(x.imag).float() for x in axs], 0
            ).to(device, dtype=self.dtype)
            # Note(kamo):
            # {'real': ..., 'imag': ...} will be changed to ComplexTensor in E2E.
            # Don't create ComplexTensor and give it E2E here
            # because torch.nn.DataParellel can't handle it.
            axs_pad = {"real": axs_pad_real, "imag": axs_pad_imag}
        else:
            axs_pad = pad_list([torch.from_numpy(x).float() for x in axs], 0).to(
                device, dtype=self.dtype
            )
            
        # perform padding and convert to tensor
        # currently only support real number
        if vxs[0].dtype.kind == "c":
            vxs_pad_real = pad_list(
                [torch.from_numpy(vx.real).float() for vx in vxs], 0
            ).to(device, dtype=self.dtype)
            vxs_pad_imag = pad_list(
                [torch.from_numpy(vx.imag).float() for vx in vxs], 0
            ).to(device, dtype=self.dtype)
            # Note(kamo):
            # {'real': ..., 'imag': ...} will be changed to ComplexTensor in E2E.
            # Don't create ComplexTensor and give it E2E here
            # because torch.nn.DataParellel can't handle it.
            vxs_pad = {"real": vxs_pad_real, "imag": vxs_pad_imag}
        else:
            vxs_pad = pad_list([torch.from_numpy(vx).float() for vx in vxs], 0).to(
                device, dtype=self.dtype
            )
        ...
        return axs_pad, vxs_pad, ilens, ys_pad
...
def train(args):
    ...
    # ]
    audio_idim_list = [
        int(valid_json[utts[0]]["input"][i]["ashape"][-1]) for i in range(args.num_encs)
    ]
    video_idim_list = [
        int(valid_json[utts[0]]["input"][i]["vshape"][-1]) for i in range(args.num_encs)
    ]
    
    odim = int(valid_json[utts[0]]["output"][0]["shape"][-1])
    for i in range(args.num_encs):
        # logging.info("stream{}: input dims : {}".format(i + 1, idim_list[i]))
        logging.info("audiostream{}: input dims : {}".format(i + 1, audio_idim_list[i]))
        logging.info("videostream{}: input dims : {}".format(i + 1, video_idim_list[i]))
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        model = load_trained_modules(audio_idim_list[0], video_idim_list[0], odim, args)
        ...
        # )
        model = model_class(
            audio_idim_list[0] if args.num_encs == 1 else audio_idim_list,
            video_idim_list[0] if args.num_encs == 1 else video_idim_list,
            odim,
            args,
        )
    # import pdb; pdb.set_trace()
    assert isinstance(model, ASRInterface)
    total_subsampling_factor = model.get_total_subsampling_factor() # 4
    ...
    if args.opt == "adadelta":
        ...
        if "transducer" in mtl_mode:
            ...
            optimizer = get_std_opt(
                model_params,
                args.adim, # 256
                args.transformer_warmup_steps, # 25000
                args.transformer_lr, # 5.0
            )
    else:
    ...
    use_sortagrad = args.sortagrad == -1 or args.sortagrad > 0 # False
    ...
    train_sampler = None
    val_sampler = None
    shuffle = not use_sortagrad # True
    if args.use_ddp:
    ...
    train_iter = ChainerDataLoader(
        dataset=train_ds,
        batch_size=1,
        num_workers=args.n_iter_processes, # 0
        shuffle=shuffle,
    ...
    updater = CustomUpdater(
        ...
        device,
        args.ngpu,
        args.grad_noise, # False
        args.accum_grad, # 8
        use_apex=use_apex, # False
        use_ddp=args.use_ddp, # False
    )
    ...
    if args.resume:
        # print(f"lgn args.resume: {args.resume}")
    ...
    if is_writable_process(args, worldsize, rank, localrank):
        ...
        if args.num_save_attention > 0 and is_attn_plot:
            data = sorted(
                ...
                key=lambda x: int(x[1]["input"][0]["ashape"][1]),
                reverse=True,
            )
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    # import pdb; pdb.set_trace()
                    nbest_hyps = model.recognize(
                        afeat, vfeat, args, train_args.char_list, rnnlm
                    )
