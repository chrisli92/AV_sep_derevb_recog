...
try:
    ...
    class PlotAttentionReport(extension.Extension):
        ...
        def trim_attention_weight(self, uttid, att_w):
            ...
            enc_len = int(self.data_dict[uttid][enc_key][enc_axis]["ashape"][0])
            if self.factor > 1:
                enc_len //= self.factor
...
try:
    ...
    class PlotCTCReport(extension.Extension):
        ...
        def trim_ctc_prob(self, uttid, prob):
            ...
            enc_len = int(self.data_dict[uttid][self.ikey][self.iaxis]["ashape"][0])
            if self.factor > 1:
                enc_len //= self.factor
            prob = prob[:enc_len]
            return prob
...
def get_model_conf(model_path, conf_path=None):
    ...
    if isinstance(confs, dict):
        ...
        # for asr, tts, mt
        idim, vidim, odim, args = confs
        return idim, vidim, odim, argparse.Namespace(**args)
