...
from espnet.online_feature_av_training.asr_utils import get_model_conf, torch_load
...
def load_trained_model(model_path, training=True):
    ...
    # )
    idim, vidim, odim, train_args = get_model_conf(
        model_path, os.path.join(os.path.dirname(model_path), "model.json")
    )
    ...
    if "transducer" in model_module:
        ...
        model = model_class(idim, vidim, odim, train_args)
...
def get_trained_model_state_dict(model_path, new_is_transducer):
    ...
    idim, vidim, odim, args = get_model_conf(model_path, conf_path)
    ...
    model_class = dynamic_import(model_module)
    model = model_class(idim, vidim, odim, args)
    torch_load(model_path, model)
...
def load_trained_modules(idim, vidim, odim, args, interface=ASRInterface):
    ...
    # import pdb; pdb.set_trace()
    main_model = model_class(idim, vidim, odim, args)
