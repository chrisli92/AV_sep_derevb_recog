...
class FeatureTransform(torch.nn.Module):
    def __init__(
        ...
        self.apply_pitch = apply_pitch

        print(f"fmin in feature transform is {fmin}, expect: {20.0}")
    ...
    def forward(
        self, x: ComplexTensor, ilens: Union[torch.LongTensor, np.ndarray, List[int]], pitch_pad
        ...
        # if self.apply_pitch:
        #     import pdb; pdb.set_trace()
        #     # concat pitch feature.
        #     # h: (B, T, F=80), pitch_pad:(B, T, F=3)  => (B, T, F=83)
        #     h = torch.concat((h, pitch_pad), dim=-1)  
        
        # import pdb; pdb.set_trace()
        # print(torch.mean(h[0, :, 0])) # check 第一个batch的第一个特征维度的均值     
...
class GlobalMVN(torch.nn.Module):
    ...
    def __init__(
        ...
        if stats_file[-3:] == "ark":
            stats = kaldiio.load_mat(stats_file)  # shape: (2,84)
            count = stats[0][-1]  # 32705614
            mean = stats[0][: -1] / count   # (83, )
    ...
    def forward(
        ...
        # import pdb; pdb.set_trace()
        if self.norm_means:
            ## to check, the value is not right.
            x += self.bias.type_as(x)[:-3]
            print(torch.mean(x[0, :, 0])) # check 第一个batch的第一个特征维度的均值
            x.masked_fill(make_pad_mask(ilens, x, 1), 0.0)

        if self.norm_vars:
            x *= self.scale.type_as(x)[:-3]
        return x, ilens
...
def feature_transform_for(args, n_fft=512):
